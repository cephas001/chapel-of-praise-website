    ...
    <div v-else class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
      <form @submit.prevent="saveSettings" class="space-y-8">
        ...
        <div class="space-y-5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="md:col-span-2">
              <label class="block text-sm font-bold text-black mb-1">Headline</label>
              <input
        ...
        <div class="space-y-5">
          <div class="flex items-center justify-between border-b border-gray-100 pb-3">
            ...
            <button type="button" @click="addChaplain" class="text-sm font-bold text-blue-600 hover:text-blue-800">
              + Add Chaplain
            </button>
          </div>
          <div class="space-y-8">
            <div v-for="(chaplain, index) in formData.chaplaincyList" :key="'chaplain' + index" class="p-5 border border-gray-200 rounded-xl bg-gray-50 relative">
              <button type="button" @click="removeChaplain(index)" class="absolute top-4 right-4 text-red-500 hover:text-red-700">
                <Icon name="heroicons:trash" class="h-5 w-5" />
              </button>
              <h4 class="text-md font-bold mb-4">Chaplain {{ index + 1 }}</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                ...
                <div class="md:col-span-2">
                  <label class="block text-sm font-bold text-black mb-1">Image URL</label>
                  <input
                ...
                <div class="md:col-span-2">
                  <label class="block text-sm font-bold text-black mb-1">Bio</label>
                  <textarea
            ...
            <div v-if="formData.chaplaincyList.length === 0" class="text-sm text-gray-500 text-center py-4">
        ...
        <div class="space-y-5">
          <div class="flex items-center justify-between border-b border-gray-100 pb-3">
            ...
            <button type="button" @click="addExecutive" class="text-sm font-bold text-blue-600 hover:text-blue-800">
              + Add Executive
            </button>
          </div>
          <div class="space-y-8">
            <div v-for="(exec, index) in formData.studentExecutivesList" :key="'exec' + index" class="p-5 border border-gray-200 rounded-xl bg-gray-50 relative">
              <button type="button" @click="removeExecutive(index)" class="absolute top-4 right-4 text-red-500 hover:text-red-700">
                <Icon name="heroicons:trash" class="h-5 w-5" />
              </button>
              <h4 class="text-md font-bold mb-4">Executive {{ index + 1 }}</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label class="block text-sm font-bold text-black mb-1">Name</label>
                  <input
        ...
        <div class="pt-6 border-t border-gray-100 flex items-center justify-end space-x-4">
          ...
          <button
            type="submit"
            class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-bold text-white bg-black rounded-xl hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            :disabled="isSaving"
          >
            <Icon
              v-if="isSaving"
